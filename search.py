...
def uniformCostSearch(problem):
    ...
    # Select top priority node
      sNode = priQueue.pop()
      state = sNode[0]
      actions = sNode[1]
    # Check if selected node is goal or not
      if problem.isGoalState(state):
          return actions
      if state not in visited:
          visited.append(state)
          for k in problem.getSuccessors(state):
              if k[0] not in visited:
                  # Calcualte cost of node and priority
                  cost = problem.getCostOfActions(actions + [k[1]])
                  priQueue.push((k[0], actions + [k[1]]), cost)
        ...
        if state not in visited:
            ...
            for k in problem.getSuccessors(state):
                if k[0] not in visited:
                    # Calcualte cost of node and priority
                    cost = problem.getCostOfActions(actions + [k[1]])
                    priQueue.push((k[0], actions + [k[1]]), cost + heuristic(k[0], problem))
